...
def rfe(X,y,model_type="logistic",keep='auto',print_support=False,print_rankings=False):
    ...
    if model_type == 'logistic':
        ...
        raise TypeError("don't know which model to pick!")
    ...
    if keep == 'auto':
        rfe_cols = [X.columns[i] for i, e in enumerate(rfe.support_) if e]
    elif isinstance(keep,int) & (keep > 0):
        rfe_cols = [X.columns[i] for i,e in enumerate(rfe.ranking_) if e <= keep]
    else:
